...
$sql = "SELECT Team,logo,comment,zip,points,points1,points2,points3 FROM upload";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		$score[$i] = $row["points"];
		$teamid2[$i] = $row["Team"];
		$score1[$i] = $row["points1"];
		$score2[$i] = $row["points2"];
		$score3[$i] = $row["points3"];
...
if(isset($_POST["publishresults"])){
						...
						= '1',points='$score1[$j]'+'$score2[$j]'+'$score3[$j]' where Team = '$teamid[$j]'" ;
...
<!-- popup form #15 -->
        ...
        <div class="popup">
		 ...
		 <form class="form-horizontal" enctype="multipart/form-data" method="POST" role="form">
			...
			<button type="submit" name="publishresults" class="btn btn-success">     Publish Result     </button>
			</div>			
			</form>
